...
class CarControllerParams:
  ...
  def __init__(self, CP, vEgoRaw=100.):
    ...
    if CP.flags & HyundaiFlags.CANFD:
      ...
      self.STEER_DELTA_UP = 10 if vEgoRaw < 15. else 3
      self.STEER_DELTA_DOWN = 10 if vEgoRaw < 15. else 4
...
class CAR(Platforms):
  ...
  HYUNDAI_IONIQ_6 = HyundaiCanFDPlatformConfig(
    ...
    CarSpecs(mass=2041, wheelbase=2.95, steerRatio=14.26, tireStiffnessFactor=0.65),
    flags=HyundaiFlags.EV | HyundaiFlags.CANFD_NO_RADAR_DISABLE,
  )
...
CANFD_SECURITYACCESS_CAR = {CAR.HYUNDAI_IONIQ_6}
